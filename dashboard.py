...
picoscope_sample_interval = 1.0            # Seconds between samples (1 Hz default)
...
def log_data(supply):
    ...
    if enable_picoscope and picoscope:
        picoscope.start_streaming(sample_interval_us=int(picoscope_sample_interval * 50))
        print(f"PicoScope streaming started at {1/picoscope_sample_interval:.0f} Hz")
    
    while not exit_event.is_set():
        time.sleep(picoscope_sample_interval)  # Use PicoScope interval for consistency
        ...
        if enable_picoscope and picoscope:
            ...
            for ch, values in stream_data.items():
                if values is not None and len(values) > 0:
                    ...
                    # Maintain RMS window
                    picoscope_rms_rolling_data[ch].append(voltage)
                    if len(picoscope_rms_rolling_data[ch]) > picoscope_rms_window:
                        picoscope_rms_rolling_data[ch].pop(0)
...
def update_picoscope_raw_plot(frame):
    """
    Update PicoScope raw data plot.
    ...
    with data_lock:
        ...
        # Plot raw PicoScope data
        if enable_picoscope:
            color_map = {'A': 'tab:orange', 'B': 'tab:purple'}
            for ch in picoscope_data:
                if picoscope_data[ch]:
                    ax_pico_raw.plot(time_data, picoscope_data[ch], 
                                   color=color_map.get(ch, 'tab:orange'),
                                   linestyle='-', linewidth=2, 
                                   label=f"Photodiode Ch{ch} Raw")
    ...
    if time_data:
        ax_pico_raw.set_xlim(max(0, time_data[-1] - 300), time_data[-1] + 10)
...
if __name__ == "__main__":
    ...
    try:
        ...
        ani_pico_raw = animation.FuncAnimation(fig, update_picoscope_raw_plot, interval=1000, cache_frame_data=False)
