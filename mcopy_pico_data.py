"""
Meghan Amendments to PicoScope Data Analysis Script
Using for fringe analysis
============================
...
INPUT DATA EXPECTED:
1. PicoScope .txt files in a subdirectory (raw waveforms)
...
file_name = 'tempsweep9_29_25'  # CHANGE THIS: Name of your data directory/file prefix
...
use_channel_b = False            # True: Analyze both Channel A and B from PicoScope
...
end_time = 2600               # End time (seconds) for analysis window
...
# MEGHAN note: this changed to volts for channel A scale - you need to change this value
peak_prominence = 20 * 10**-3            # Minimum peak height (mV) to be counted as a real peak
...
if os.path.exists(sm_csv_name):
    ...
    try:
        ...
        if not test_files:
            raise FileNotFoundError(f"No .txt files found in directory: {directory_path}")
        ...
        if os.path.basename(test_files[0])[9:11] == "00":
            ...
            all_files = sorted(test_files, key=lambda f: int(os.path.basename(f)[17:19]))
    ...
    for file_idx, file in enumerate(all_files):
        ...
        print(f'MEGHAN columns: {initial_data.columns}')
